...
Nowadays, it is common that a single machine is equiped more than one GPUs. The
following figure shows that 4 GPUs are connected to the GPU through a PCIe
switch.

![](img/multi-gpu.svg)

If nvidia driver is installed, we can check how many GPUs are avaialbe by
`nvidia-smi`. Note that running this tutorial requires at least 2 GPUs.
...
Using all of GPUs together often significantly speedup the training. Different
to the CPU that all CPU cores are used in default, however, most layers can only
run on a single GPU. Therefore, additional works are required to partition
workloads into multiple GPUs.

## Data Parallelism

Data parallelism is the widely used approach to partition workloads. Assume
there are *k* GPUs, it split the examples in a data batch into *k* parts, each
GPU will compute the gradient on a part of the batch, gradients are then summed
across all GPUs before updating the weights.
...
We first demonstrate how to run workloads in parallel. Writing parallel codes in
Python in non-trivial, but fortunately, MXNet is able to automatically
parallelize the workloads. Two technologies are used to achieve this goal.

Firstly, workloads, such as `nd.dot` are pushed into the backend engine for
*lazy evaluation*. That is, Python merely pushes the workload `nd.dot` and
returns immediately without waiting for the computation is finished. We keep
pushing until the results need to be copied out from MXNet, such as `print(x)`
or converting into numpy by `x.asnumpy()`. At that time, the Python thread is
blocked until the results are ready.
...
Secondly, the backend engine will analysis the dependencies of the pushed
workloads. If two workloads are independent to each other, then the engine may
...
Then `a` and `b` may run in parallel, while `c` needs to wait both `a` and `b`
are ready.

The following codes show the engine effectively parallelize the `dot`s on two
GPUs
...
We will use the convolutional neural networks and plain SGD introduced in [cnn-
scratch]() as an example workload.
...
The following function copy the parameters into a particular GPU and initialize
the gradients.
...
Given a list of data that span over multiple GPUs, we define a function to sum
the data and then broadcast the results to each GPU.
...
Given a data batch, we define a function that splits this batch and copies each
part into the according GPU.
...
For inference, we simply let it run on the first GPU. We leave a data
parallelism implementation as an excise.
...
Running on multiple GPUs, we often want to increase the batch size so that each
GPU still gets a enough large batch size for good computation performance. A
larger batch size sometimes slow down the convergence, we often want to
increases the learning rate as well.
...
We shown how to develop a data parallelism training program from scratch. Thanks
to the auto-parallelism, we only need to write serial codes while the engine is
able to parallelize them on multiple GPUs.
